...
class Engine(abstract.Engine):
    def __init__(self, url: str = None, core_engine: sqlalchemy.engine.Engine = None, *args, **kwargs):
    ...
    def create_metadata(self) -> sqlalchemy.MetaData:
        metadata = sqlalchemy.MetaData(bind=self._core_engine)
        return metadata
...
class Repository(abstract.Repository):
    ...
    def _get(self, model_class: type[models.BaseModel], **identities,) -> list[models.BaseModel]:
        ...
        if identities is not None:
            models_ = self.session.scalars(sqlalchemy.select(model_class).filter_by(**identities)).all()
        else:
            models_ = self.session.scalars(sqlalchemy.select(model_class)).all()
        return models_

class Session(abstract.Session):
    def __init__(self, core_session: sqlalchemy_orm.Session, *args, **kwargs,):
...
class SessionFactory:
    def __init__(self, url: str = None, engine: Engine | sqlalchemy.engine.Engine = None, **kwargs):
        if engine is None:
            if url is None:
                raise ValueError("Either url or engine must be provided")
            engine = Engine(url, **kwargs)
        self.engine = engine
        self.__core_factory = sqlalchemy_orm.sessionmaker(bind=engine._core_engine, **kwargs)
